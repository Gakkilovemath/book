...
List ComputeMLE(NumericVector T, IntegerMatrix frequencies, int K)
{
    ...
    T1 = new double[ndata+1];
    
    freq = new int *[K+1];
    
    for (i=0;i<K+1;i++)
        freq[i]= new int[ndata+1];
    
    for (i=1;i<=ndata;i++)
    {
        T1[i]=(double)T[i-1];
        for (k=0;k<=K;k++)
          freq[k][i]=(int)frequencies(i-1,k);
    }
    ...
    //cerr << setw(10) << N << setw(10) << ndata << endl;
    ...
    for (i=0;i<K+2;i++)
    {
        F[i]= new double[ndata+1];
        ind[i]= new int[ndata+1];
    }
    ...
    ICM(N,ndata,freq,K,ind,F,n_Iterations,&phi,&iterations,tol);
    ...
    delete[] F, delete[] ind, delete[] p, delete[] jumploc, delete[] freq, delete[] njumps;
